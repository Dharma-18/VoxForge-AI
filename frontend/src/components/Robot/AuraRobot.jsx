...
function RobotModel({ state }) {
  const { scene } = useGLTF('/models/aura.glb')
  ...
  useFrame(({ clock }) => {
    ...
    if (state === 'idle') {
      robotRef.current.position.y = Math.sin(time * 1.1) * 0.04
      robotRef.current.rotation.y = Math.sin(time * 0.4) * 0.06
    }
    if (state === 'listening') {
      robotRef.current.position.y = Math.sin(time * 2.3) * 0.06
      robotRef.current.rotation.y = Math.sin(time * 2) * 0.08
    }
    if (state === 'thinking') {
      robotRef.current.rotation.z = Math.sin(time * 1.8) * 0.05
      robotRef.current.rotation.x = Math.sin(time * 1.3) * 0.04
    }
    if (state === 'talking') {
      robotRef.current.rotation.x = Math.sin(time * 5) * 0.05
      robotRef.current.position.y = Math.sin(time * 4.8) * 0.03
    }
    if (state === 'executing') {
      robotRef.current.rotation.y = Math.sin(time * 4.2) * 0.1
    }
  })

  return (
    <group ref={robotRef} position={[0, -0.9, 0.5]} scale={[1.2, 1.2, 1.2]}>
...
const Box = forwardRef(function Box({ args, position, children, ...props }, ref) {
  return (
...
const Sphere = forwardRef(function Sphere(
  { args, position, children, ...props },
  ref,
) {
  return (
    <mesh ref={ref} position={position} {...props}>
...
export default function AuraRobot() {
  ...
  const screenState = useStore((state) => state.screenState)
  ...
  return (
    <div className="w-full h-full relative">
      ...
      >
        ...
        <DeskSetup state={robotState} screenState={screenState} />
        <HolographicAura state={robotState} />
        <Suspense fallback={<RobotBody state={robotState} screenState={screenState} />}>
          <RobotModel state={robotState} />
        </Suspense>
